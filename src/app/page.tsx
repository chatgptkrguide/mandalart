    ...
    <div className="min-h-screen">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="flex items-end justify-between mb-6">
          <div>
            <h1 className="text-lg sm:text-xl font-bold leading-tight">내 만다라트</h1>
            <p className="text-xs text-[var(--color-text-muted)] mt-0.5">
              {mine.length > 0 ? `${mine.length}개 진행 중` : '목표를 만들어보세요'}
            </p>
          </div>
          <button onClick={() => router.push('/mandalart/new')} className="btn btn-fill btn-sm">
            + 새로 만들기
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center py-16 anim-fade">
            <div className="w-20 h-20 bg-[var(--color-bg-warm)] rounded-2xl flex items-center justify-center mb-5">
              <div className="grid grid-cols-3 gap-0.5">
                {Array.from({ length: 9 }).map((_, i) => (
                  <div key={i} className={`w-3.5 h-3.5 rounded-sm ${i === 4 ? 'bg-[var(--color-primary)]' : 'bg-[var(--color-border)]'}`} />
                ))}
              </div>
            </div>
            <p className="text-sm font-medium mb-1">아직 만다라트가 없어요</p>
